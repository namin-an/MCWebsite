...
<script>
    ...
    function chk(txt){
        ...
        if(selAns == <?php echo $_SESSION['Answer']?>){
            ...
            location.href = 'SetConfidence_2.php?tm='+tm_toint+'&corr_cnt=1&selec='+selAns;
        }else{
            let time = $('#clock').val();
            let tm_totxt = time+"."+mil;
            let tm_toint = parseFloat(tm_totxt).toFixed(2);
            location.href = 'SetConfidence_2.php?tm='+tm_toint+'&incorr_cnt=1&selec='+selAns;
        }
    }
    ...
    window.onload = function() {
        ...
        if(width == 0){
            location.replace('TimeOut_A_2.php');
        }
        setTimeout(()=>{
            location.href='TimeOut_A_2.php';
        }, 5000);
    }
