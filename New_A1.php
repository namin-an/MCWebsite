...
$query = "select MC1 from MainTest where idx = '$QuestionNum'";
...
<script>
    ...
    function chk(txt){
        ...
        if(selAns == <?php echo $_SESSION['Answer']?>){
            alert('정답');
            let time = $('#clock').val();
            location.href = 'NewConf.php?tm='+time+'&corr_cnt=1&selec='+selAns;
        }else{
            alert('오답');
            let time = $('#clock').val();
            location.href = 'NewConf.php?tm='+time+'&incorr_cnt=1&selec='+selAns;
        }
